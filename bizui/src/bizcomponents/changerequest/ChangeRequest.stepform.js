...
const buildSteps = (targetComponent, stepCode) => {
  ...
  const chainCodeInvokerStepOf = GlobalComponents.ChainCodeInvokerBase.stepOf;

  const { ChangeRequestCreateFormBody } = GlobalComponents;
  const { ChainCodeInvokerCreateFormBody } = GlobalComponents;

  const stepCollection = {};

  stepCollection['CERT_CHECKER'] = [
    changeRequestStepOf(
      targetComponent,
      '创建变更请求',
      <ChangeRequestCreateFormBody {...targetComponent.props} hideTitle />,
      '',
      0
    ),
    ,
  ...
  stepCollection['NODE_ROLE_MANAGER'] = [
    changeRequestStepOf(
      targetComponent,
      '创建变更请求',
      <ChangeRequestCreateFormBody {...targetComponent.props} hideTitle />,
  ...
  stepCollection['CC_INVOKER'] = [
    ...
    chainCodeInvokerStepOf(
      targetComponent,
      '创建链代码调用程序',
      <ChainCodeInvokerCreateFormBody {...targetComponent.props} hideTitle />,
...
class ChangeRequestStepForm extends React.Component {
  ...
  render() {
    ...
    const { code } = urlContext();

    const steps = buildSteps(this, code);
      ...
      <PageHeaderLayout title={renderTitle()}>
        <Card>
          <div>
            ...
            <div className={styles.stepsContent} style={{ marginBottom: '24px' }}>
