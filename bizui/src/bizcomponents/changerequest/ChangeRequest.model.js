...
export default {
  ...
  effects: {
    ...
    *addChainCodeInvoker({ payload }, { call, put }) {
      ...
      const data = yield call(ChangeRequestService.addChainCodeInvoker, id, parameters);
      if (hasError(data)) {
      ...
      const location = {
        pathname: `/changeRequest/${id}/list/ChainCodeInvokerList/链代码调用程序+${appLocaleName(
          userContext,
          'List'
        )}`,
        state: newState,
    ...
    *updateChainCodeInvoker({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters);
      const data = yield call(ChangeRequestService.updateChainCodeInvoker, id, parameters);
      if (hasError(data)) {
        handleServerError(data);
        return;
      }
      ...
      const location = {
        pathname: `/changeRequest/${id}/list/ChainCodeInvokerList/链代码调用程序列表`,
        state: newPlayload,
      };
      yield put(routerRedux.push(location));
    },
    ...
    *removeChainCodeInvokerList({ payload }, { call, put }) {
      ...
      const { id, role, parameters, continueNext } = payload;
      console.log('get form parameters', parameters);
      const data = yield call(ChangeRequestService.removeChainCodeInvokerList, id, parameters);
      if (hasError(data)) {
        handleServerError(data);
        return;
      }
