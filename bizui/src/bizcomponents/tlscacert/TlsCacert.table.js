...
import styles from './TlsCacert.table.less'
...
class TlsCacertTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = TlsCacertBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/tlsCacert/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
