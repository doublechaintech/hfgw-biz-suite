...
import styles from './TransactionStatus.search.less';
...
const getValue = obj =>
  Object.keys(obj)
    .map(key => obj[key])
    .join(',');
const { fieldLabels } = TransactionStatusBase;
...
class TransactionStatusSearchForm extends PureComponent {
  state = {
    // addInputValue: '',
    // modalVisible: false,
    expandForm: false,
  ...
  buildStringSearchParameters = (formValues, fieldName) => {
    ...
    return {
      transactionStatusList: 1,
      'transactionStatusList.searchField': fieldName,
      'transactionStatusList.searchVerb': 'startsWith',
      'transactionStatusList.searchValue': fieldValue,
    }
  }
  */
  ...
  handleSearch = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      pushIfNotNull(
        paramList,
        this.buildStringSearchParameters(listName, fieldsValue, 'contains', 'code')
      );
      pushIfNotNull(
        paramList,
      ...
      dispatch({
        ...
        payload: {
          id: owner.id,
          parameters: params,
          transactionStatusSearchFormParameters: filterObjectKeys(fieldsValue),
            ...
            <Form.Item label={fieldLabels.network}>
              {getFieldDecorator('network', { initialValue: tryinit('network') })(
                <SelectObject
                  ...
                  targetType={'network'}
                  requestFunction={TransactionStatusService.requestCandidateNetwork}
                  useForSearch
                />
              )}
            </Form.Item>
...
export default Form.create()(TransactionStatusSearchForm);
