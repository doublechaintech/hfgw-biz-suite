...
import styles from './ServiceRecord.updateform.less';
...
class ServiceRecordUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const serviceRecordId = values.id;
        const imagesValues = mapBackToImageValues(convertedImagesValues);
        const parameters = { ...values, serviceRecordId, ...imagesValues };
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateServiceRecord`,
          payload: {
            id: owner.id,
            type: 'serviceRecord',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1;
      dispatch({
        type: `${owner.type}/gotoNextServiceRecordUpdateRow`,
        payload: {
          id: owner.id,
          type: 'serviceRecord',
          selectedRows,
    ...
    const internalRenderTitle = () => {
      ...
      return (
        <div>
          ...
          服务记录: {currentUpdateIndex + 1}/{selectedRows.length}
        </div>
      );
    };
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item>
                  {getFieldDecorator('payload', {
                    initialValue: selectedRow.payload,
                    rules: [{ required: true, message: appLocaleName(userContext, 'PleaseInput') }],
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item>
                  {getFieldDecorator('response', {
                    initialValue: selectedRow.response,
                    rules: [{ required: true, message: appLocaleName(userContext, 'PleaseInput') }],
                  })(<TextArea rows={4} placeholder={appLocaleName(userContext, 'PleaseInput')} />)}
                </Form.Item>
              </Col>
...
}))(Form.create()(ServiceRecordUpdateForm));
