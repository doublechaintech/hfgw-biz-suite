...
const requestCandidateApplication = (ownerClass, id, filterKey, pageNo) => {
 
  const url = `${PREFIX}serviceRecordManager/requestCandidateApplication/ownerClass/id/filterKey/pageNo/`
  const requestParameters = {id, ownerClass,filterKey, pageNo}
  return postForm({url,requestParameters})
}	

const transferToAnotherApplication = (id, parameters) => {
  const url = `${PREFIX}serviceRecordManager/transferToAnotherApplication/id/anotherApplicationId/`
  const requestParameters = {id, ...parameters}
  return postForm({url,requestParameters})
}
...
const ServiceRecordService = { view,
  load,
