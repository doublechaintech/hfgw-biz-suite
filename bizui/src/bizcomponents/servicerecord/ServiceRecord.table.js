...
class ServiceRecordTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/serviceRecord/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
