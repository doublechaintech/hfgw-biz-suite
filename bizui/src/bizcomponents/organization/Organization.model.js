...
export default {
  ...
  effects: {
    ...
    *addNode({ payload }, { call, put }) {
      ...
      const data = yield call(OrganizationService.addNode, id, parameters)
      if (hasError(data)) {
      ...
      const location = { pathname: `/organization/${id}/list/NodeList/节点+${appLocaleName(userContext,'List')}`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateNode({ payload }, { call, put }) {
      ...
      console.log('get form parameters', parameters)
      const data = yield call(OrganizationService.updateNode, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
      ...
      const location = { pathname: `/organization/${id}/list/NodeList/节点列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
    ...
    *removeNodeList({ payload }, { call, put }) {
     const userContext = null
      ...
      const { id, role, parameters, continueNext } = payload
      console.log('get form parameters', parameters)
      const data = yield call(OrganizationService.removeNodeList, id, parameters)
      if (hasError(data)) {
        handleServerError(data)
        return
      }
