...
const renderSettingMenuItem = (item, cardsData, targetComponent) => {
  ...
  return (
    <Menu.Item key={item.name}>
      <Link
        to={`/formField/${targetComponent.props.formField.id}/list/${item.name}/${
          item.displayName
        }/`}
      >
  ...
  return (
    <Menu>
      <Menu.Item key="profile">
        <Link to={`/formField/${targetComponent.props.formField.id}/permission`}>
...
const internalSummaryOf = (formField, targetComponent) => {
  ...
  return (
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="标签" style={{ wordBreak: 'break-all' }}>
        {formField.label}
      </Description>
      <Description term="语言环境的关键" style={{ wordBreak: 'break-all' }}>
        {formField.localeKey}
      </Description>
      <Description term="参数名称" style={{ wordBreak: 'break-all' }}>
        {formField.parameterName}
      </Description>
      <Description term="类型" style={{ wordBreak: 'break-all' }}>
        {formField.type}
      </Description>
      <Description term="占位符" style={{ wordBreak: 'break-all' }}>
        {formField.placeholder}
      </Description>
      <Description term="默认值" style={{ wordBreak: 'break-all' }}>
        {formField.defaultValue}
      </Description>
      <Description term="描述" style={{ wordBreak: 'break-all' }}>
        {formField.description}
      </Description>
      <Description term="字段组" style={{ wordBreak: 'break-all' }}>
        {formField.fieldGroup}
      </Description>
      <Description term="最小值" style={{ wordBreak: 'break-all' }}>
        {formField.minimumValue}
      </Description>
      <Description term="最大值" style={{ wordBreak: 'break-all' }}>
        {formField.maximumValue}
      </Description>
      <Description term="候选人的价值观" style={{ wordBreak: 'break-all' }}>
        {formField.candidateValues}
      </Description>
      <Description term="建议值" style={{ wordBreak: 'break-all' }}>
