...
const menuData = {
  menuName: window.trans('change_request_type'),
  menuFor: 'changeRequestType',
  subItems: [
    {
      name: 'changeRequestList',
      displayName: window.mtrans(
        'change_request',
        'change_request_type.change_request_list',
        false
      ),
      type: 'changeRequest',
      icon: 'exchange-alt',
...
const fieldLabels = {
  ...
  network: window.trans('change_request_type.network'),
};
...
const displayColumns = [
  {
    ...
    dataIndex: 'id',
    width: '8',
    render: (text, record) => renderTextCell(text, record, 'changeRequestType'),
    sorter: true,
  },
  {
  ...
  {
    title: fieldLabels.displayOrder,
    debugtype: 'int',
  ...
  {
    title: fieldLabels.stepConfiguration,
    debugtype: 'string_longtext',
    dataIndex: 'stepConfiguration',
    width: '10',
    render: (text, record) => renderTextCell(text, record),
  },
  {
  ...
  return (
    <div key={changeRequestType.id}>
      <DescriptionList key={changeRequestType.id} size="small" col="2">
        ...
        <Description term={fieldLabels.icon} style={{ wordBreak: 'break-all' }}>
          {changeRequestType.icon}
        </Description>
        <Description term={fieldLabels.displayOrder}>
          <div style={{ color: 'red' }}>{changeRequestType.displayOrder}</div>
        </Description>
      </DescriptionList>
