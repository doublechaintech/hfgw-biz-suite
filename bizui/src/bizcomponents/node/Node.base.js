...
const menuData = {menuName:"节点", menuFor: "node",
  ...
  {name: 'channelPeerRoleList', displayName:'通道对等的角色', icon:'500px',readPermission: false,createPermission: false,deletePermission: false,updatePermission: false,executionPermission: false, viewGroup: '__no_group'},
...
const displayColumns = [
  ...
  { title: fieldLabels.tlsCacert, debugtype: 'string_longtext', dataIndex: 'tlsCacert', width: '10',render: (text, record)=>renderTextCell(text,record)},
...
const packFormValuesToObject = ( formValuesToPack )=>{
	const {name, url, address, contactPerson, contactTelephone, organizationId, channelId, networkId, typeId, tlsCacert} = formValuesToPack
	...
	const data = {name, url, address, contactPerson, contactTelephone, organization, channel, network, type, tlsCacert}
	return data
}
const unpackObjectToFormValues = ( objectToUnpack )=>{
	const {name, url, address, contactPerson, contactTelephone, organization, channel, network, type, tlsCacert} = objectToUnpack
	const organizationId = organization ? organization.id : null
	const channelId = channel ? channel.id : null
	const networkId = network ? network.id : null
	const typeId = type ? type.id : null
	const data = {name, url, address, contactPerson, contactTelephone, organizationId, channelId, networkId, typeId, tlsCacert}
	return data
}
