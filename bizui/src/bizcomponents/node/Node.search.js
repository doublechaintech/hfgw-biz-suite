  ...
  return (
    <div>
      <OrganizationAssociateForm
        visible={currentAssociateModal === 'organization'}
        data={{ nodeList: selectedRows }}
        owner={owner}
        onCancel={() => toggleAssociateModalVisible(targetComponent, 'organization')}
        onCreate={() => toggleAssociateModalVisible(targetComponent, 'organization')}
      />{' '}
...
class NodeSearch extends PureComponent {
  state = {
  ...
  render() {
    ...
    const { NodeTable } = GlobalComponents;
    const { NodeSearchForm } = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={renderTitle()}>
        ...
        {showDeletionDialog(this, NodeModalTable, 'nodeIds')}
...
export default Form.create()(NodeSearch);
