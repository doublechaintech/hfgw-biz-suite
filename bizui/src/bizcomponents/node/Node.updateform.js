...
import styles from './Node.updateform.less';
...
class NodeUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const parameters = { ...values, nodeId, ...imagesValues };
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1;
      dispatch({
        type: `${owner.type}/gotoNextNodeUpdateRow`,
        payload: {
          id: owner.id,
          type: 'node',
          selectedRows,
    ...
    const internalRenderTitle = () => {
      ...
      return (
        <div>
          ...
          节点: {currentUpdateIndex + 1}/{selectedRows.length}
        </div>
      );
    };
    ...
    return (
      ...
      >
        ...
        <Card title={window.trans('node')} className={styles.card} bordered={false}>
          <Form>
            <Row gutter={16}>
              <Col lg={24} md={24} sm={24}>
                <Form.Item>
                  {getFieldDecorator('tlsCacert', {
                    initialValue: selectedRow.tlsCacert,
                    rules: [{ required: true, message: appLocaleName(userContext, 'PleaseInput') }],
