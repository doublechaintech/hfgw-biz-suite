...
import NodeCreateFormBody from './Node.createformbody';
...
class NodeCreateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitCreateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const parameters = { ...values, ...imagesValues };
        dispatch({
          type: `${owner.type}/addNode`,
          payload: { id: owner.id, type: 'node', parameters, continueNext: true },
        });
      });
    };
    ...
    const goback = () => {
      ...
      dispatch({
        ...
        payload: { id: owner.id, type: 'node', listName: appLocaleName(userContext, 'List') },
      });
    };
    ...
    return (
      <PageHeaderLayout
        ...
        content={`${appLocaleName(userContext, 'CreateNew')}${window.trans('node')}`}
        wrapperClassName={styles.advancedForm}
      >
        <NodeCreateFormBody {...this.props} />
