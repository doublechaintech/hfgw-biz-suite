...
import styles from './PeerRole.table.less'
...
class PeerRoleTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = PeerRoleBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/peerRole/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
