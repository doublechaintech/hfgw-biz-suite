...
import styles from './PeerRole.updateform.less';
...
class PeerRoleUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const peerRoleId = values.id;
        const imagesValues = mapBackToImageValues(convertedImagesValues);
        const parameters = { ...values, peerRoleId, ...imagesValues };
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updatePeerRole`,
          payload: {
            id: owner.id,
            type: 'peerRole',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1;
      dispatch({
        type: `${owner.type}/gotoNextPeerRoleUpdateRow`,
        payload: {
          id: owner.id,
          type: 'peerRole',
          selectedRows,
    ...
    const internalRenderTitle = () => {
      ...
      return (
        <div>
          ...
          对等的角色: {currentUpdateIndex + 1}/{selectedRows.length}
        </div>
      );
    };
...
}))(Form.create()(PeerRoleUpdateForm));
