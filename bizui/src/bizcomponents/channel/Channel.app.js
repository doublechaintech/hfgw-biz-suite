...
class ChannelBizApp extends React.PureComponent {
    ...
    return (
      ...
      >
        <Menu.Item key="dashboard">
          <Link to={`/channel/${this.props.channel.id}/dashboard`}>
  ...
  buildRouters = () => {
    ...
    const routers = [
      ...
      {
        path: '/channel/:id/list/channelPeerRoleCreateForm',
        component: this.getChannelPeerRoleCreateForm(),
      },
      {
        path: '/channel/:id/list/channelPeerRoleUpdateForm',
  ...
  render() {
    ...
    const { searchLocalData } = GlobalComponents.ChannelBase;
        ...
        <Layout style={{ marginTop: 44 }}>
          {this.state.showSearch && (
            <div style={{ backgroundColor: 'black' }} onClick={() => hideSearchResult()}>
              {searchLocalData(this.props.channel, this.state.searchKeyword)}
            </div>
          )}
