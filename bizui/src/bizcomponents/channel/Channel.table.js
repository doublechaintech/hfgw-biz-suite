...
import styles from './Channel.table.less'
...
class ChannelTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = ChannelBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/channel/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
