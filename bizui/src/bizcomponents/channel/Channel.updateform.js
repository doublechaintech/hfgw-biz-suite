...
import styles from './Channel.updateform.less';
...
class ChannelUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const parameters = { ...values, channelId, ...imagesValues };
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1;
      dispatch({
        type: `${owner.type}/gotoNextChannelUpdateRow`,
        payload: {
          id: owner.id,
          type: 'channel',
          selectedRows,
    ...
    const internalRenderTitle = () => {
      ...
      return (
        <div>
          ...
          频道: {currentUpdateIndex + 1}/{selectedRows.length}
        </div>
      );
    };
