...
import styles from './ChainCodeInvoker.updateform.less';
...
class ChainCodeInvokerUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const chainCodeInvokerId = values.id;
        const imagesValues = mapBackToImageValues(convertedImagesValues);
        const parameters = { ...values, chainCodeInvokerId, ...imagesValues };
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateChainCodeInvoker`,
          payload: {
            id: owner.id,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1;
      dispatch({
        type: `${owner.type}/gotoNextChainCodeInvokerUpdateRow`,
        payload: {
          id: owner.id,
          type: 'chainCodeInvoker',
          selectedRows,
    ...
    const internalRenderTitle = () => {
      ...
      return (
        <div>
          ...
          链代码调用程序: {currentUpdateIndex + 1}/{selectedRows.length}
        </div>
      );
    };
    ...
    return (
      ...
      >
        ...
        <Card title={window.trans('chain_code_invoker')} className={styles.card} bordered={false}>
          <Form>
            <Row gutter={16}>
              <Col lg={24} md={24} sm={24}>
                <Form.Item>
                  {getFieldDecorator('parameters', {
                    initialValue: selectedRow.parameters,
                    rules: [{ required: true, message: appLocaleName(userContext, 'PleaseInput') }],
...
}))(Form.create()(ChainCodeInvokerUpdateForm));
