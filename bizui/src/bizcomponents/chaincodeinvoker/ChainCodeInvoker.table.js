...
import styles from './ChainCodeInvoker.table.less';
...
class ChainCodeInvokerTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  };
  ...
  enhanceColumnsWithSorter = () => {
    const { displayColumns } = ChainCodeInvokerBase;
    ...
    const remainColumns = displayColumns.filter(
      (item, index) =>
        item.dataIndex !== referenceName && index < 10 && item.dataIndex !== 'content'
    );
  ...
  calcDisplayColumns = () => {
    ...
    const operationColumn = {
      ...
      render: (text, record) => (
        <span>
          {hasReadPermission(metaInfo) && (
            <Link to={`/chainCodeInvoker/${record.id}/dashboard`}>
              {appLocaleName(userContext, 'View')}
            </Link>
          )}
          {hasUpdatePermission(metaInfo) && <span className={styles.splitLine} />}{' '}
          {hasUpdatePermission(metaInfo) && (
            <a key="__2" onClick={() => this.gotoEdit(text, record)}>
              {appLocaleName(userContext, 'Edit')}
            </a>
          )}
          {record.actionList &&
            record.actionList.map(item => (
              <a key={item.actionId} onClick={() => this.executeAction(item, text, record)}>
    ...
    enhancedColumns.push(operationColumn);
          ...
          <Alert
            message={
              selectedRowKeys.length === 0 ? (
                <span>
                  {appLocaleName(userContext, 'Totally')} <a style={{ fontWeight: 600 }}>{count}</a>{' '}
                  {appLocaleName(userContext, 'Items')},{' '}
                  {appLocaleName(userContext, 'PleaseSelectItemtoProceed')}
                </span>
              ) : (
                <span>
                  {appLocaleName(userContext, 'Totally')} <a style={{ fontWeight: 600 }}>{count}</a>{' '}
                  {appLocaleName(userContext, 'Items')}
                  {appLocaleName(userContext, 'Selected')}{' '}
                  <a style={{ fontWeight: 600 }}>{selectedRowKeys.length}</a>{' '}
                  {appLocaleName(userContext, 'Items')}{' '}
                  <a onClick={this.cleanSelectedKeys} style={{ marginLeft: 24 }}>
                    {appLocaleName(userContext, 'Clear')}
                  </a>
                </span>
              )
            }
...
export default ChainCodeInvokerTable;
