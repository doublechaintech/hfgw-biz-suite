...
const imageList = quickLink => {
  return [{ title: '图片路径', imageLocation: quickLink.imagePath }];
};

const internalImageListOf = quickLink => defaultImageListOf(quickLink, imageList);

const optionList = quickLink => {
  return [];
};
...
const renderSettingMenuItem = (item, cardsData, targetComponent) => {
  ...
  return (
    <Menu.Item key={item.name}>
      <Link
        to={`/quickLink/${targetComponent.props.quickLink.id}/list/${item.name}/${
          item.displayName
        }/`}
      >
  ...
  return (
    <Menu>
      <Menu.Item key="profile">
        <Link to={`/quickLink/${targetComponent.props.quickLink.id}/permission`}>
...
const internalSummaryOf = (quickLink, targetComponent) => {
  ...
  return (
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="链接的目标" style={{ wordBreak: 'break-all' }}>
        {quickLink.linkTarget}
      </Description>
      <Description term="创建时间">
        {moment(quickLink.createTime).format('YYYY-MM-DD HH:mm')}
      </Description>
      <Description term="应用程序">
        {quickLink.app == null
          ? appLocaleName(userContext, 'NotAssigned')
          : `${quickLink.app.displayName}(${quickLink.app.id})`}
        <Icon
          type="swap"
          onClick={() =>
            showTransferModel(
              ...
              '应用程序',
              'userApp',
              QuickLinkService.requestCandidateApp,
              QuickLinkService.transferToAnotherApp,
              'anotherAppId',
              quickLink.app ? quickLink.app.id : ''
            )
          }
...
class QuickLinkDashboard extends Component {
  ...
  render() {
    ...
    const cardsData = {
      cardsName: '快速链接',
      cardsFor: 'quickLink',
      cardsSource: this.props.quickLink,
      returnURL,
      displayName,
      subItems: [],
      subSettingItems: [],
