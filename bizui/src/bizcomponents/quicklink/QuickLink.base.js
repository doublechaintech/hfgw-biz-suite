...
const menuData = {
  menuName: window.trans('quick_link'),
  menuFor: 'quickLink',
  subItems: [],
};
...
const displayColumns = [
  {
    ...
    dataIndex: 'id',
    width: '8',
    render: (text, record) => renderTextCell(text, record, 'quickLink'),
    sorter: true,
  },
  {
  ...
  {
    title: fieldLabels.imagePath,
    dataIndex: 'imagePath',
    render: (text, record) => renderImageCell(text, record, 'quick_link.image_path'),
  },
  {
    title: fieldLabels.linkTarget,
    debugtype: 'string',
  ...
  return (
    <div key={quickLink.id}>
      <DescriptionList key={quickLink.id} size="small" col="2">
        ...
        <Description term={fieldLabels.linkTarget} style={{ wordBreak: 'break-all' }}>
          {quickLink.linkTarget}
        </Description>
        <Description term={fieldLabels.createTime}>
          <div>{moment(quickLink.createTime).format('YYYY-MM-DD HH:mm')}</div>
        </Description>
