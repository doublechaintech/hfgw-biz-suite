...
const menuData = {
  menuName: window.trans('sec_user_blocking'),
  menuFor: 'secUserBlocking',
  subItems: [
    {
      name: 'secUserList',
      displayName: window.mtrans('sec_user', 'sec_user_blocking.sec_user_list', false),
      type: 'secUser',
      icon: 'user',
  ...
  return (
    <div key={secUserBlocking.id}>
      <DescriptionList key={secUserBlocking.id} size="small" col="2">
        ...
        <Description term={fieldLabels.who} style={{ wordBreak: 'break-all' }}>
          {secUserBlocking.who}
        </Description>
        <Description term={fieldLabels.blockTime}>
          <div>{moment(secUserBlocking.blockTime).format('YYYY-MM-DD HH:mm')}</div>
        </Description>
        <Description term={fieldLabels.comments} style={{ wordBreak: 'break-all' }}>
          {secUserBlocking.comments}
        </Description>
      </DescriptionList>
