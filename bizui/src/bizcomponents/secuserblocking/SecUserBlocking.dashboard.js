...
const renderSettingMenuItem = (item, cardsData, targetComponent) => {
  ...
  return (
    <Menu.Item key={item.name}>
      <Link
        to={`/secUserBlocking/${targetComponent.props.secUserBlocking.id}/list/${item.name}/${
          item.displayName
        }/`}
      >
  ...
  return (
    <Menu>
      <Menu.Item key="profile">
        <Link to={`/secUserBlocking/${targetComponent.props.secUserBlocking.id}/permission`}>
...
const internalSummaryOf = (secUserBlocking, targetComponent) => {
  ...
  return (
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="谁" style={{ wordBreak: 'break-all' }}>
        {secUserBlocking.who}
      </Description>
      <Description term="块时间">
        {moment(secUserBlocking.blockTime).format('YYYY-MM-DD HH:mm')}
      </Description>
      <Description term="评论" style={{ wordBreak: 'break-all' }}>
