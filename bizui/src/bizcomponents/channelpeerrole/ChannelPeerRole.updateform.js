...
import styles from './ChannelPeerRole.updateform.less';
...
class ChannelPeerRoleUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const channelPeerRoleId = values.id;
        const imagesValues = mapBackToImageValues(convertedImagesValues);
        const parameters = { ...values, channelPeerRoleId, ...imagesValues };
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateChannelPeerRole`,
          payload: {
            id: owner.id,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1;
      dispatch({
        type: `${owner.type}/gotoNextChannelPeerRoleUpdateRow`,
        payload: {
          id: owner.id,
          type: 'channelPeerRole',
          selectedRows,
    ...
    const internalRenderTitle = () => {
      ...
      return (
        <div>
          ...
          通道对等的角色: {currentUpdateIndex + 1}/{selectedRows.length}
        </div>
      );
    };
...
}))(Form.create()(ChannelPeerRoleUpdateForm));
