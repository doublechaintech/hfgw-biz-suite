...
const showAssociateDialog = (targetComponent) => {
  ...
  const { NodeAssociateForm } = GlobalComponents
  const { PeerRoleAssociateForm } = GlobalComponents
  ...
  <div>
	...
	data={{channelPeerRoleList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'channel')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'channel')}/> <NodeAssociateForm 
	visible={currentAssociateModal==='node'} 
	data={{channelPeerRoleList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'node')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'node')}/> <PeerRoleAssociateForm 
	visible={currentAssociateModal==='peerRole'} 
	data={{channelPeerRoleList:selectedRows}} owner={owner}  
	onCancel={()=>toggleAssociateModalVisible(targetComponent,'peerRole')} 
	onCreate={()=>toggleAssociateModalVisible(targetComponent,'peerRole')}/> 
...
class ChannelPeerRoleSearch extends PureComponent {
  state = {
  ...
  render(){
    ...
    const {ChannelPeerRoleTable} = GlobalComponents;
    const {ChannelPeerRoleSearchForm} = GlobalComponents;
    ...
    return (
      <PageHeaderLayout title={renderTitle()}>
        <Card bordered={false}>
          <div className={styles.tableList}>
            <div className={styles.tableListForm}>
              <ChannelPeerRoleSearchForm {...this.props} />
            </div>
        ...
        {showDeletionDialog(this,ChannelPeerRoleModalTable,"channelPeerRoleIds")}
...
export default Form.create()(ChannelPeerRoleSearch)
