...
import styles from './ChannelPeerRole.search.less'
...
class ChannelPeerRoleSearchForm extends PureComponent {
  state = {
    // addInputValue: '',
    // modalVisible: false,
    expandForm: false,
...
componentDidMount() {
  ...
  buildStringSearchParameters = (formValues, fieldName) => {
    ...
    return {
      channelPeerRoleList: 1,
      'channelPeerRoleList.searchField': fieldName,
      'channelPeerRoleList.searchVerb': 'startsWith',
      'channelPeerRoleList.searchValue': fieldValue,
    }
  }
  */
  ...
  handleSearch = (e) => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      const {listName} = owner
		...
		pushIfNotNull(paramList,this.buildStringSearchParameters(listName, fieldsValue,'eq', 'node'))
		pushIfNotNull(paramList,this.buildStringSearchParameters(listName, fieldsValue,'eq', 'peerRole'))
      ...
      dispatch({
        ...
        channelPeerRoleSearchFormParameters: filterObjectKeys(fieldsValue),
 ...
 <Col md={8} sm={24}>
                 <Form.Item label="对等的角色">
               {getFieldDecorator('peerRole', {initialValue: tryinit('peerRole')})(
               
               <SelectObject 
                 disabled={!availableForEdit('peerRole')}
                 targetType={"peerRole"} 
                 requestFunction={ChannelPeerRoleService.requestCandidatePeerRole} useForSearch />
 ...
 <Col md={8} sm={24}>
                  ...
                  <SelectObject 
                    ...
                    targetType={"channel"} 
                    requestFunction={ChannelPeerRoleService.requestCandidateChannel} useForSearch />
 ...
 <Col md={8} sm={24}>
                    <Form.Item label="节点">
                  {getFieldDecorator('node', {initialValue: tryinit('node')})(
                  
                  <SelectObject 
                    disabled={!availableForEdit('node')}
                    targetType={"node"} 
                    requestFunction={ChannelPeerRoleService.requestCandidateNode} useForSearch />
...
export default Form.create()(ChannelPeerRoleSearchForm)
