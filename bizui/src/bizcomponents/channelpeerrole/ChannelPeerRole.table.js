...
import styles from './ChannelPeerRole.table.less'
...
class ChannelPeerRoleTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = ChannelPeerRoleBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/channelPeerRole/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
...
export default ChannelPeerRoleTable
