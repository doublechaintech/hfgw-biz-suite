...
const renderSettingMenuItem = (item, cardsData, targetComponent) => {
  ...
  return (
    <Menu.Item key={item.name}>
      <Link
        to={`/loginHistory/${targetComponent.props.loginHistory.id}/list/${item.name}/${
          item.displayName
        }/`}
      >
  ...
  return (
    <Menu>
      <Menu.Item key="profile">
        <Link to={`/loginHistory/${targetComponent.props.loginHistory.id}/permission`}>
  ...
  return (
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="登录时间">
        {moment(loginHistory.loginTime).format('YYYY-MM-DD HH:mm')}
      </Description>
      <Description term="来自IP" style={{ wordBreak: 'break-all' }}>
        {loginHistory.fromIp}
      </Description>
