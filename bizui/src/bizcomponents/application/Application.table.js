...
import styles from './Application.table.less'
...
class ApplicationTable extends PureComponent {
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = ApplicationBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/application/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
