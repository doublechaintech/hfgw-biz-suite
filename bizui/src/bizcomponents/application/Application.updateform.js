...
import styles from './Application.updateform.less'
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const parameters = { ...values, applicationId, ...imagesValues }
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateApplication`,
          payload: {
            id: owner.id,
            type: 'application',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1
      dispatch({
        type: `${owner.type}/gotoNextApplicationUpdateRow`,
        payload: {
          id: owner.id,
          type: 'application',
          selectedRows,
	...
	const internalRenderTitle = () =>{
      ...
      return (<div>{linkComp}{appLocaleName(userContext,"Update")}应用程序: {(currentUpdateIndex+1)}/{selectedRows.length}</div>)
    }
	...
	return (
      ...
      >
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.mspid} {...formItemLayout}>
                  {getFieldDecorator('mspid', {
                    initialValue: selectedRow.mspid,
                    rules: [{ required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
       ...
       <Card title={`公钥`} className={styles.card} bordered={false}>
          <Form >
          	<Row gutter={16}> 
              <Col lg={24} md={24} sm={24}>
                <Form.Item>
       ...
       <Card title={`私钥`} className={styles.card} bordered={false}>
          <Form >
          	<Row gutter={16}> 
              <Col lg={24} md={24} sm={24}>
                <Form.Item>
                  {getFieldDecorator('privateKey', {
                  	initialValue: selectedRow.privateKey,
                    rules: [{  required: true, message: appLocaleName(userContext,"PleaseInput") }],
                  })(
                    <TextArea rows={4} placeholder={appLocaleName(userContext,"PleaseInput")} />
                  )}
...
}))(Form.create()(ApplicationUpdateForm))
