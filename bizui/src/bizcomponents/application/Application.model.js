...
export default {
  ...
  effects: {
    ...
    *addServiceRecord({ payload }, { call, put }) {
      ...
      const location = { pathname: `/application/${id}/list/ServiceRecordList/服务记录+${appLocaleName(userContext,'List')}`, state: newState }
      yield put(routerRedux.push(location))
    },
    *updateServiceRecord({ payload }, { call, put }) {
      ...
      const location = { pathname: `/application/${id}/list/ServiceRecordList/服务记录列表`, state: newPlayload }
      yield put(routerRedux.push(location))
    },
