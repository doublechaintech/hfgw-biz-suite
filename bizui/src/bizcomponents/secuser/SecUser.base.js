...
const menuData = {
  menuName: window.trans('sec_user'),
  menuFor: 'secUser',
  subItems: [
    {
      name: 'userAppList',
      displayName: window.mtrans('user_app', 'sec_user.user_app_list', false),
      type: 'userApp',
      icon: 'user',
    ...
    },
    {
      ...
      displayName: window.mtrans('login_history', 'sec_user.login_history_list', false),
      type: 'loginHistory',
      icon: 'history',
      readPermission: false,
      createPermission: false,
  ...
  return (
    <div key={secUser.id}>
      <DescriptionList key={secUser.id} size="small" col="2">
        ...
        <Description term={fieldLabels.login} style={{ wordBreak: 'break-all' }}>
          {secUser.login}
        </Description>
        <Description term={fieldLabels.mobile} style={{ wordBreak: 'break-all' }}>
          {secUser.mobile}
        </Description>
        <Description term={fieldLabels.email} style={{ wordBreak: 'break-all' }}>
          {secUser.email}
        </Description>
        <Description term={fieldLabels.pwd} style={{ wordBreak: 'break-all' }}>
          {secUser.pwd}
        </Description>
        <Description term={fieldLabels.weixinOpenid} style={{ wordBreak: 'break-all' }}>
          {secUser.weixinOpenid}
        </Description>
        <Description term={fieldLabels.weixinAppid} style={{ wordBreak: 'break-all' }}>
          {secUser.weixinAppid}
        </Description>
        <Description term={fieldLabels.accessToken} style={{ wordBreak: 'break-all' }}>
          {secUser.accessToken}
        </Description>
        <Description term={fieldLabels.verificationCode}>
          <div style={{ color: 'red' }}>{secUser.verificationCode}</div>
        </Description>
        <Description term={fieldLabels.verificationCodeExpire}>
          <div>{moment(secUser.verificationCodeExpire).format('YYYY-MM-DD HH:mm')}</div>
        </Description>
        <Description term={fieldLabels.lastLoginTime}>
          <div>{moment(secUser.lastLoginTime).format('YYYY-MM-DD HH:mm')}</div>
        </Description>
      </DescriptionList>
