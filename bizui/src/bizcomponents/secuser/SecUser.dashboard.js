...
const renderSettingMenuItem = (item, cardsData, targetComponent) => {
  ...
  return (
    <Menu.Item key={item.name}>
      <Link
        to={`/secUser/${targetComponent.props.secUser.id}/list/${item.name}/${item.displayName}/`}
      >
  ...
  return (
    <Menu>
      <Menu.Item key="profile">
        <Link to={`/secUser/${targetComponent.props.secUser.id}/permission`}>
...
const internalSummaryOf = (secUser, targetComponent) => {
  ...
  return (
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="登录" style={{ wordBreak: 'break-all' }}>
        {secUser.login}
      </Description>
      <Description term="手机号码" style={{ wordBreak: 'break-all' }}>
        {secUser.mobile}
      </Description>
      <Description term="电子邮件" style={{ wordBreak: 'break-all' }}>
        {secUser.email}
      </Description>
      <Description term="密码" style={{ wordBreak: 'break-all' }}>
        {secUser.pwd}
      </Description>
      <Description term="微信openid" style={{ wordBreak: 'break-all' }}>
        {secUser.weixinOpenid}
      </Description>
      <Description term="微信Appid" style={{ wordBreak: 'break-all' }}>
        {secUser.weixinAppid}
      </Description>
      <Description term="访问令牌" style={{ wordBreak: 'break-all' }}>
        {secUser.accessToken}
      </Description>
      <Description term="验证码" style={{ wordBreak: 'break-all' }}>
        {secUser.verificationCode}
      </Description>
      <Description term="验证码过期">
        {moment(secUser.verificationCodeExpire).format('YYYY-MM-DD HH:mm')}
      </Description>
      <Description term="最后登录时间">
        {moment(secUser.lastLoginTime).format('YYYY-MM-DD HH:mm')}
      </Description>
