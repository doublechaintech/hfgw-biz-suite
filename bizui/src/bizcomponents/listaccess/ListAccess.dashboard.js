...
const renderSettingMenuItem = (item, cardsData, targetComponent) => {
  ...
  return (
    <Menu.Item key={item.name}>
      <Link
        to={`/listAccess/${targetComponent.props.listAccess.id}/list/${item.name}/${
          item.displayName
        }/`}
      >
  ...
  return (
    <Menu>
      <Menu.Item key="profile">
        <Link to={`/listAccess/${targetComponent.props.listAccess.id}/permission`}>
...
const internalSummaryOf = (listAccess, targetComponent) => {
  ...
  return (
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="内部名称" style={{ wordBreak: 'break-all' }}>
