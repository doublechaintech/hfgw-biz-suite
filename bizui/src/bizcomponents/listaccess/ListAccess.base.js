...
const menuData = {
  menuName: window.trans('list_access'),
  menuFor: 'listAccess',
  subItems: [],
};
...
const displayColumns = [
  {
    ...
    dataIndex: 'id',
    width: '8',
    render: (text, record) => renderTextCell(text, record, 'listAccess'),
    sorter: true,
  },
  {
  ...
  return (
    <div key={listAccess.id}>
      <DescriptionList key={listAccess.id} size="small" col="2">
        ...
        <Description term={fieldLabels.internalName} style={{ wordBreak: 'break-all' }}>
          {listAccess.internalName}
        </Description>
