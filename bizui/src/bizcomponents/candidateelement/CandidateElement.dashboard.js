...
const imageList = candidateElement => {
  return [{ title: '图片', imageLocation: candidateElement.image }];
};

const internalImageListOf = candidateElement => defaultImageListOf(candidateElement, imageList);

const optionList = candidateElement => {
  return [];
};
...
const internalSettingListOf = candidateElement =>
  defaultSettingListOf(candidateElement, optionList);
...
const renderSettingMenuItem = (item, cardsData, targetComponent) => {
  ...
  return (
    <Menu.Item key={item.name}>
      <Link
        to={`/candidateElement/${targetComponent.props.candidateElement.id}/list/${item.name}/${
          item.displayName
        }/`}
      >
  ...
  return (
    <Menu>
      <Menu.Item key="profile">
        <Link to={`/candidateElement/${targetComponent.props.candidateElement.id}/permission`}>
...
class CandidateElementDashboard extends Component {
  ...
  render() {
    ...
    const cardsData = {
      cardsName: '候选人元素',
      cardsFor: 'candidateElement',
      cardsSource: this.props.candidateElement,
      returnURL,
      displayName,
      subItems: [],
      subSettingItems: [],
