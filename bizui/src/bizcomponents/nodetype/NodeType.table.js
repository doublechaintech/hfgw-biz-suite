...
import styles from './NodeType.table.less'
...
class NodeTypeTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = NodeTypeBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/nodeType/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
