...
const addNode = (targetObjectId, parameters) => {
  const url = `${PREFIX}nodeTypeManager/addNode/nodeTypeId/name/url/organizationId/channelId/tokensExpr/`
...
const updateNode = (targetObjectId, parameters) => {
  const url = `${PREFIX}nodeTypeManager/updateNodeProperties/nodeTypeId/id/name/url/tokensExpr/`
  const nodeTypeId = targetObjectId
  const requestParameters = { ...parameters, nodeTypeId, tokensExpr: 'none' }
  return postForm({ url,requestParameters})
}
