...
import styles from './NodeType.updateform.less';
...
class NodeTypeUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const nodeTypeId = values.id;
        const imagesValues = mapBackToImageValues(convertedImagesValues);
        const parameters = { ...values, nodeTypeId, ...imagesValues };
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateNodeType`,
          payload: {
            id: owner.id,
            type: 'nodeType',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1;
      dispatch({
        type: `${owner.type}/gotoNextNodeTypeUpdateRow`,
        payload: {
          id: owner.id,
          type: 'nodeType',
          selectedRows,
    ...
    const internalRenderTitle = () => {
      ...
      return (
        <div>
          ...
          节点类型: {currentUpdateIndex + 1}/{selectedRows.length}
        </div>
      );
    };
...
}))(Form.create()(NodeTypeUpdateForm));
