...
import styles from './ChainCode.dashboard.less';
...
const imageList = chainCode => {
  return [];
};

const internalImageListOf = chainCode => defaultImageListOf(chainCode, imageList);

const optionList = chainCode => {
  return [];
};
...
const internalSettingListOf = chainCode => defaultSettingListOf(chainCode, optionList);
const internalLargeTextOf = chainCode => {
  return null;
};
...
const renderSettingMenuItem = (item, cardsData, targetComponent) => {
  ...
  return (
    <Menu.Item key={item.name}>
      <Link
        to={`/chainCode/${targetComponent.props.chainCode.id}/list/${item.name}/${
          item.displayName
        }/`}
      >
  ...
  return (
    <Menu>
      <Menu.Item key="profile">
        <Link to={`/chainCode/${targetComponent.props.chainCode.id}/permission`}>
...
const internalSummaryOf = (chainCode, targetComponent) => {
  ...
  return (
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="代号" style={{ wordBreak: 'break-all' }}>
        {chainCode.codeName}
      </Description>
      <Description term="代码版本" style={{ wordBreak: 'break-all' }}>
        ...
        <Icon
          ...
          onClick={() =>
            showTransferModel(
              ...
              '频道',
              'channel',
              ChainCodeService.requestCandidateChannel,
              ChainCodeService.transferToAnotherChannel,
              'anotherChannelId',
              chainCode.channel ? chainCode.channel.id : ''
            )
          }
      ...
      {buildTransferModal(chainCode, targetComponent)}
    </DescriptionList>
  );
};
...
class ChainCodeDashboard extends Component {
  state = {
  ...
  render() {
    ...
    } = this.props.chainCode;
    if (!this.props.chainCode.class) {
      return null;
    }
    ...
    const cardsData = {
      cardsName: '链码',
      cardsFor: 'chainCode',
      cardsSource: this.props.chainCode,
      returnURL,
      displayName,
      subItems: [
        {
...
}))(Form.create()(ChainCodeDashboard));
