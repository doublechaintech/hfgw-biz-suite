...
import styles from './ChainCode.table.less'
...
class ChainCodeTable extends PureComponent {
  state = {
    selectedRowKeys: [],
  }
  ...
  enhanceColumnsWithSorter=()=>{
    const {displayColumns} = ChainCodeBase
    ...
    const operationColumn={
      ...
      render: (text, record) => (
        <span>
          ...
          { hasReadPermission(metaInfo)&&<Link to={`/chainCode/${record.id}/dashboard`}>{appLocaleName(userContext,"View")}</Link>}
