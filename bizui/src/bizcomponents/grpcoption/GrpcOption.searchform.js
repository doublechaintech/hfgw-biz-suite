...
import styles from './GrpcOption.search.less';
import GlobalComponents from '../../custcomponents';
import SelectObject from '../../components/SelectObject';
import appLocaleName from '../../common/Locale.tool';
import GrpcOptionBase from './GrpcOption.base';
...
const getValue = obj =>
  Object.keys(obj)
    .map(key => obj[key])
    .join(',');
const { fieldLabels } = GrpcOptionBase;
...
class GrpcOptionSearchForm extends PureComponent {
  state = {
    // addInputValue: '',
    // modalVisible: false,
    expandForm: false,
  ...
  buildStringSearchParameters = (formValues, fieldName) => {
    ...
    return {
      grpcOptionList: 1,
      'grpcOptionList.searchField': fieldName,
      'grpcOptionList.searchVerb': 'startsWith',
      'grpcOptionList.searchValue': fieldValue,
    }
  }
  */
  ...
  handleSearch = e => {
    ...
    form.validateFields((err, fieldsValue) => {
      ...
      pushIfNotNull(
        paramList,
        this.buildStringSearchParameters(listName, fieldsValue, 'contains', 'parameterValue')
      );
      pushIfNotNull(
        paramList,
        this.buildStringSearchParameters(listName, fieldsValue, 'eq', 'node')
      );
      ...
      dispatch({
        ...
        payload: {
          id: owner.id,
          parameters: params,
          grpcOptionSearchFormParameters: filterObjectKeys(fieldsValue),
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          ...
          <Col md={8} sm={24}>
            <FormItem label={fieldLabels.parameterValue}>
          ...
          <Col md={8} sm={24}>
            <Form.Item label={fieldLabels.node}>
              {getFieldDecorator('node', { initialValue: tryinit('node') })(
                <SelectObject
                  disabled={!availableForEdit('node')}
                  targetType={'node'}
                  requestFunction={GrpcOptionService.requestCandidateNode}
                  useForSearch
                />
              )}
            </Form.Item>
...
export default Form.create()(GrpcOptionSearchForm);
