...
import styles from './GrpcOption.updateform.less';
...
class GrpcOptionUpdateForm extends Component {
  state = {
  ...
  render() {
    ...
    const submitUpdateForm = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        const grpcOptionId = values.id;
        const imagesValues = mapBackToImageValues(convertedImagesValues);
        const parameters = { ...values, grpcOptionId, ...imagesValues };
    ...
    const submitUpdateFormAndContinue = () => {
      validateFieldsAndScroll((error, values) => {
        ...
        dispatch({
          type: `${owner.type}/updateGrpcOption`,
          payload: {
            id: owner.id,
            type: 'grpcOption',
            parameters,
    ...
    const skipToNext = () => {
      ...
      const newIndex = currentUpdateIndex + 1;
      dispatch({
        type: `${owner.type}/gotoNextGrpcOptionUpdateRow`,
        payload: {
          id: owner.id,
          type: 'grpcOption',
          selectedRows,
    ...
    const internalRenderTitle = () => {
      ...
      return (
        <div>
          ...
          Grpc选项: {currentUpdateIndex + 1}/{selectedRows.length}
        </div>
      );
    };
        ...
        >
          <Form>
            <Row gutter={16}>
              ...
              <Col lg={24} md={24} sm={24}>
                <Form.Item label={fieldLabels.parameterValue} {...formItemLayout}>
                  {getFieldDecorator('parameterValue', {
                    initialValue: selectedRow.parameterValue,
                    rules: [{ required: true, message: appLocaleName(userContext, 'PleaseInput') }],
                  })(<Input size="large" placeHolder={fieldLabels.parameterValue} />)}
                </Form.Item>
              </Col>
            </Row>
          </Form>
...
}))(Form.create()(GrpcOptionUpdateForm));
