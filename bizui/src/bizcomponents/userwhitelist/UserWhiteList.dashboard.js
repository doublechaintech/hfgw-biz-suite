...
const renderSettingMenuItem = (item, cardsData, targetComponent) => {
  ...
  return (
    <Menu.Item key={item.name}>
      <Link
        to={`/userWhiteList/${targetComponent.props.userWhiteList.id}/list/${item.name}/${
          item.displayName
        }/`}
      >
  ...
  return (
    <Menu>
      <Menu.Item key="profile">
        <Link to={`/userWhiteList/${targetComponent.props.userWhiteList.id}/permission`}>
...
const internalSummaryOf = (userWhiteList, targetComponent) => {
  ...
  return (
    <DescriptionList className={styles.headerList} size="small" col="4">
      ...
      <Description term="用户身份" style={{ wordBreak: 'break-all' }}>
        {userWhiteList.userIdentity}
      </Description>
      <Description term="用户特殊功能" style={{ wordBreak: 'break-all' }}>
