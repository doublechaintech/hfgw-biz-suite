...
const menuData = {
  menuName: window.trans('user_white_list'),
...
const displayColumns = [
  {
    ...
    dataIndex: 'id',
    width: '8',
    render: (text, record) => renderTextCell(text, record, 'userWhiteList'),
    sorter: true,
  },
  {
  ...
  return (
    <div key={userWhiteList.id}>
      <DescriptionList key={userWhiteList.id} size="small" col="2">
        ...
        <Description term={fieldLabels.userIdentity} style={{ wordBreak: 'break-all' }}>
          {userWhiteList.userIdentity}
        </Description>
        <Description term={fieldLabels.userSpecialFunctions} style={{ wordBreak: 'break-all' }}>
          {userWhiteList.userSpecialFunctions}
        </Description>
      </DescriptionList>
